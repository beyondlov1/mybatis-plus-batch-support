        ...
        if (StringUtils.isNotBlank(tableInfo.getKeyProperty())) {
            if (tableInfo.getIdType() == IdType.AUTO) {
                ...
                if (null != tableInfo.getKeySequence()) {
                    keyGenerator = TableInfoHelper.genKeyGenerator(CustomSqlMethod.INSERT_SET_COLUMNS_BATCH.getMethod(), tableInfo, builderAssistant);
                    keyProperty = tableInfo.getKeyProperty();
                    keyColumn = tableInfo.getKeyColumn();
                }
            }
        }
        return this.addInsertMappedStatement(mapperClass, modelClass, CustomSqlMethod.INSERT_SET_COLUMNS_BATCH.getMethod(), sqlSource, keyGenerator, keyProperty, keyColumn);
    }

    private String getBatchInsertSql(TableInfo tableInfo) {
        String batchInsertSql = CustomSqlMethod.INSERT_SET_COLUMNS_BATCH.getSql();
